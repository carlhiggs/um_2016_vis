...
output$map <-  renderLeaflet({
  ...
  pal <- colorNumeric(palette = "RdYlGn", domain = c(0,max(active_data()@data[,3:8],na.rm=TRUE)))
...
'* only data coverage area is used as basis for percentage calculations.<br> Modified Mesh Blocks upon which area estimates are based were restricted to include only those with data coverage of more than 90%',
'</td></tr></table>')
...
output$barplot=renderPlot({
  ...
  output$pieplot=renderPlot({
    ...
    if(is.null(area_of_interest)){
        area_stats=as.numeric(active_summary()[,c('peranytree_public','peranytree_private','peranytree_other')])
